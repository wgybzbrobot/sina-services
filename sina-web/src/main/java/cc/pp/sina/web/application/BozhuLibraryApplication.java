 ...
 *
 */
public class BozhuLibraryApplication extends Application {

	private static Logger logger = LoggerFactory.getLogger(BozhuLibraryApplication.class);

	private final AnalysisInfo bozhusInfoByUid;
	private final BozhuQuery bozhuQuery;

	public BozhuLibraryApplication(AnalysisInfo bozhusInfoByUid, BozhuQuery bozhuQuery) {
	...
	public Restlet createInboundRoot() {
		...
		router.attach("/users/{uids}/info", AnalysisResource.class);
		router.attach("/users/{uids}/collect", AnalysisCollectedResource.class);
		router.attach("/users/bozhu", QueryByBozhuResource.class);
		router.attach("/users/fans", QueryByFansResource.class);
		return router;
	}
	...
	public List<UserAllParamsDomain> getBozhusInfo(List<String> uids) {
		...
		List<UserAllParamsDomain> result = new ArrayList<>();
