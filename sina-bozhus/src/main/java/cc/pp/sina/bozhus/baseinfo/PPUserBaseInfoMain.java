...
public class PPUserBaseInfoMain {

	private static Logger logger = LoggerFactory.getLogger(PPUserBaseInfoMain.class);
	...
	public static void main(String[] args) {
		...
		try {
			for (Long uid : tokenService.getSinaUids()) {
				if (!pool.isShutdown()) {
					pool.execute(new UserBaseInfoRun(weiboJDBC, sinaUserInfoDao, uid.toString()));
				} else {
